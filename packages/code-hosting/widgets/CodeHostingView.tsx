...
export function CodeHostingView({ renderDetail }: {
  ...
  // The counter must render from the same filtered collection as the rows, or a
  // tab can claim more than it lists (e.g. Draft/Ready narrow client-side on top
  // of the provider's `state=open` superset). The inactive tab keeps its loaded
  // total: selecting it resets the filter to "all" and refetches that list.
  const issueCount = tab === "issues" ? visibleItems.length : issues.length;
  const changeCount = tab === "changes" ? visibleItems.length : changes.length;
  ...
  // List/empty-state heading. Preferred plural, but an older provider that only
  // declares the prose singular still compiles and renders its existing label.
  const issuePlural = provider.presentation.issuePlural ?? provider.presentation.issueLabel;
  ...
  return <div className="github-page">
    ...
    {status?.repo && status.authenticated && <>
      ...
      <div className="gh-list-controls"><Tabs className="gh-tabs" size="sm" label={provider.presentation.serviceName} value={tab} tabs={[{ id: "issues", label: <>{issuePlural}{!loading && <> <span>{issueCount}</span></>}</> }, { id: "changes", label: <>{provider.presentation.changePlural}{!loading && <> <span>{changeCount}</span></>}</> }]} onChange={(value: string) => { setTab(value as CodeHostingTab); setFilter("all"); }} /><div className="source-search gh-search">{Icon.search?.()}<TextInput value={query} placeholder={t("githubview.searchValue", { value: tab === "issues" ? issuePlural : provider.presentation.changePlural })} aria-label={t("githubview.searchValue", { value: tab === "issues" ? issuePlural : provider.presentation.changePlural })} onChange={(event: ChangeEvent<HTMLInputElement>) => setQuery(event.target.value)} />{query && <IconButton className="source-search-clear" icon={Icon.close} label={t("githubview.clearSearch")} onClick={() => setQuery("")} />}</div><div className="gh-filter-chips ui-scroll-tabs" aria-label={t("githubview.filterList")}>{filters.map((item) => <Button size="sm" variant="ghost" key={item.id} className={filter === item.id ? "active" : ""} aria-pressed={filter === item.id} onClick={() => setFilter(item.id)}>{item.label}</Button>)}</div></div>
      ...
      {!loading && !reason && visibleItems.length === 0 && <EmptyState title={query || filter !== "all" ? t("githubview.noMatchingResults") : t("githubview.noValue", { value: tab === "issues" ? issuePlural : provider.presentation.changePlural })} description={query || filter !== "all" ? t("githubview.tryADifferentSearchOr") : t("githubview.newRepositoryActivityWillAppearHere")} {...(query || filter !== "all" ? { actionLabel: t("githubview.clearFilters"), onAction: () => { setQuery(""); setFilter("all"); } } : {})} />}

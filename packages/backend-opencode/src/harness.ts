import type { AgentRuntime, HarnessContext, HarnessDiscovery, HarnessProvider } from "@polyth/contracts";
import { CAPABILITIES } from "./index.ts";
import { inspectOpenCodeEngine, resolveOpenCodeBinary } from "./runtimeStorage.ts";

type RuntimeResolver = (context: HarnessContext) => Promise<AgentRuntime>;

const discoverRuntime = async (
    context: HarnessContext,
    resolveRuntime: RuntimeResolver,
): Promise<HarnessDiscovery> => {
    const engine = await resolveRuntime(context);
...
export function createOpenCodeHarness(
    ...
    /** Read-only fallback with authority separate from a project executor.
    ...
    return {
        ...
        async discover(context) {
            try {
                return await discoverRuntime(context, runtime);
            }
            catch (primaryError) {
                if (!discoveryFallback) throw primaryError;
                return discoverRuntime(context, (fallbackContext) =>
                    discoveryFallback(fallbackContext, primaryError));
            }
        },

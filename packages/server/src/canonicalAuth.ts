 ...
 * principal contract. Loopback address is reachability information only unless
 * the operator explicitly enables the development-only agent bypass. That
 * bypass is still gated by the server-minted direct-loopback ingress marker.
 */
export function createCanonicalAuthGateway(options: {
  control: ControlPlane;
  identity: IdentityService;
  cookieName: string;
  /** Development-only: unauthenticated direct loopback requests act as the active owner. */
  ...
  const debugOwnerPrincipal = (): AuthPrincipal | null => {
    if (!options.debugAgentAccess) return null;
  ...
  const gateway: CanonicalAuthGateway = {
    ...
    refreshPrincipal(principal) {
      switch (principal.kind) {
        ...
        case "local-user": {
          const userId = directUser(principal);
          return options.debugAgentAccess && userId && activeOwnerId() === userId ? principal : null;
        }

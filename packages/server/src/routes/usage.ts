...
import { buildProviderUsageOverview, type UsageService } from "@polyth/usage";
...
export function usageRoutes(usage: UsageService): RouteHandler {
  return async ({ path, method, json, body }) => {
    if (path === "/api/usage/quotas" && method === "GET") {
      ...
      const overview = new Map(buildProviderUsageOverview(snapshots).map((item) => [item.providerId, item]));
      json(200, snapshots.map((s) => ({
        ...s,
        pace: usage.pace(s.providerId),
        overview: overview.get(s.providerId),
      })));
      return true;
    }

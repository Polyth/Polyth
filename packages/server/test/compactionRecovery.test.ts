...
test("first turn in a fresh runtime epoch restores active goal and pins", async () => {
  ...
  try {
    ...
    assert.match(turns[1]!, /The execution runtime was replaced/);
    assert.match(turns[1]!, /Conversation context was reconstructed from confirmed Polyth history/);
    assert.match(turns[1]!, /Do not assume unconfirmed operations from the previous runtime completed/);
    ...
    const recoveryContext = (recovered.data as { recoveryContext?: string }).recoveryContext;
    assert.equal(typeof recoveryContext, "string");
    assert.match(recoveryContext!, /The execution runtime was replaced/);
    assert.match(recoveryContext!, /Remember this confirmed fact/);
    assert.ok(turns[1]!.startsWith(recoveryContext!));
    assert.equal(
...
test("a second runtime epoch never rehydrates an older fenced turn tail", async () => {
  ...
  try {
    ...
    assert.doesNotMatch(turns[2]!, /uncertain request/);
    assert.doesNotMatch(
      turns[2]!,
      /UNTRUSTED PARTIAL/,
      "a fenced prior-epoch tail must not reappear after a later runtime loss",
    );
    const second = (await store.events(created.id))
      .filter((event) => event.type === "user/message")
      .at(-1)!;
    assert.match(
      (second.data as { recoveryContext?: string }).recoveryContext ?? "",
...
function epochHarness(dir: string) {
  const listeners = new Set<Emit>();
  const turns: string[] = [];
  let backendSessionId = "backend-old";
  let comparisonOrder = 0;
  ...
  const runtime: AgentRuntime = {
    ...
    branchSession: async () => `backend-branch-${++comparisonOrder}`,
    abort: async () => undefined,
    replyPermission: async () => undefined,
    replyQuestion: async () => undefined,
    endpoint: async () => endpoint,
    ...
    reconcile: async (
      ...
      state: {
        value: "idle",
        comparison: { domain: "epoch-structured", order: ++comparisonOrder },
      },
      completeness: {
        events: "partial",
        permissions: "partial",
  ...
  return {
    ...
    emit,
    runtime,
    replace: async (nextBackend: string) => {
      endpoint = {
        ...endpoint,
        authorityId: `owned:${nextBackend}`,
        generation: 1,
        url: `http://runtime.invalid/${nextBackend}`,
        control: { kind: "owned", instanceToken: nextBackend },
      };
      backendSessionId = nextBackend;
    },
    oldAuthority: () => ({ authorityId: "owned:old", generation: 4 }),
  };
}

test("epoch recovery respects rewind-effective history and rides user/message.recoveryContext", async () => {
  const dir = mkdtempSync(join(tmpdir(), "polyth-epoch-rewind-"));
  const store = createStore(join(dir, "sessions.db"));
  const harness = epochHarness(dir);
  const project: Project = { id: "p-rewind", name: "Rewind", path: dir, createdAt: 1 };
  const sessions = createSessionService({
    store,
    projects: {
      list: async () => [project],
  ...
  try {
    const created = await sessions.create({ projectId: project.id, title: "Rewind epoch" });
    await sessions.send(created.id, { text: "keep this prefix" });
    harness.emit(created.id, { type: "turn/stopped", reason: "completed" });
    await flush();
    await sessions.send(created.id, { text: "HIDDEN_REWIND_TAIL" });
    harness.emit(created.id, { type: "turn/stopped", reason: "completed" });
    await flush();
    const hide = (await store.events(created.id)).filter((event) => event.type === "user/message").at(-1)!;
    await sessions.rewind!(created.id, hide.seq);
    await harness.replace("backend-rewound");
    const reset = await store.prepareOperation({
      sessionId: created.id,
      mutationKind: "session-reset",
      intentEvent: {
    ...
    await store.settleOperation(reset.operation.operationId, {
      kind: "confirmed",
      receipt: "backend-rewound",
    });
    await sessions.transitionRuntimeEpoch(created.id, harness.runtime, {
      resetOperationId: reset.operation.operationId,
      reason: "runtime storage was replaced",
      authorityDisposition: {
        kind: "owned-authority-destroyed",
        ...harness.oldAuthority(),
      },
    });

    await sessions.send(created.id, { text: "continue after rewind" });
    const recovered = (await store.events(created.id))
      .filter((event) => event.type === "user/message")
      .at(-1)!;
    const recoveryContext = (recovered.data as { recoveryContext?: string }).recoveryContext ?? "";
    assert.equal((recovered.data as { text?: string }).text, "continue after rewind");
    assert.match(recoveryContext, /keep this prefix/);
    assert.doesNotMatch(recoveryContext, /HIDDEN_REWIND_TAIL/);
    assert.match(recoveryContext, /The execution runtime was replaced/);
    assert.ok(harness.turns.at(-1)!.includes(recoveryContext));
    assert.match(harness.turns.at(-1)!, /\n\ncontinue after rewind$/);
  } finally {
    await store.close();
    rmSync(dir, { recursive: true, force: true });
  }
...
test("epoch recovery keeps pins and recent tail inside the 16k cap with an omission footer", async () => {
  const dir = mkdtempSync(join(tmpdir(), "polyth-epoch-budget-"));
  const store = createStore(join(dir, "sessions.db"));
  const harness = epochHarness(dir);
  const project: Project = { id: "p-budget", name: "Budget", path: dir, createdAt: 1 };
  const sessions = createSessionService({
    store,
    projects: {
      list: async () => [project],
    ...
    hooks: {
      runtimeEpochContext: async (_sessionId, events) => ({
        objective: "GOAL-KEEP reserved budget",
        pinned: activePinnedMessages(events),
      }),
    },
  });
  ...
  try {
    const created = await sessions.create({ projectId: project.id, title: "Budget epoch" });
    for (let i = 0; i < 16; i++) {
      const pin = await store.append(created.id, "user/message", {
        text: `PIN-${i} ${"p".repeat(450)}`,
      });
      await sessions.pinContext!(created.id, pin.seq);
    }
    for (let i = 0; i < 40; i++) {
      await store.append(created.id, "user/message", { text: `TAIL-${i} ${"t".repeat(450)}` });
      await store.append(created.id, "assistant/message", {
        partId: `a-${i}`,
        text: `reply-${i}`,
      });
    }
    await harness.replace("backend-budget");
    const reset = await store.prepareOperation({
      sessionId: created.id,
      mutationKind: "session-reset",
      intentEvent: {
    ...
    await sessions.send(created.id, { text: "continue after budget" });
    const recovered = (await store.events(created.id))
      .filter((event) => event.type === "user/message")
      .at(-1)!;
    const recoveryContext = (recovered.data as { recoveryContext?: string }).recoveryContext ?? "";
    assert.ok(recoveryContext.length <= 16_000);
    assert.match(recoveryContext, /GOAL-KEEP/);
    assert.match(recoveryContext, /PIN-/);
    assert.match(recoveryContext, /TAIL-39/);
    assert.match(recoveryContext, /Omitted:/);
    assert.doesNotMatch(recoveryContext, /TAIL-0 /);
    assert.equal((recovered.data as { text?: string }).text, "continue after budget");
    assert.ok(harness.turns.at(-1)!.startsWith(recoveryContext));
  } finally {
    await store.close();
    rmSync(dir, { recursive: true, force: true });
  }

...
function OpenCodeRuntime({ snapshot }: { snapshot?: HarnessSnapshot }) {
  ...
  useEffect(() => {
    ...
    return () => { window.clearInterval(timer); window.removeEventListener("polyth:opencode-pending", refresh); };
  }, []);
  const apply = async () => {
    setBusy(true);
    setError("");

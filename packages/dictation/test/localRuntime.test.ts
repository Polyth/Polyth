...
test("concurrent runtime downloads share one upstream flight and cancel aborts without deleting state", async () => {
  let fetches = 0;
  let aborts = 0;
  const dir = root();
  try {
    ...
    assert.notEqual((await runtime.status()).state, "downloading");

    // remove remains the destructive cleanup path and is safe after cancel.

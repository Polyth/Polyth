...
test("verified resource adoption becomes a clean second capsule that can activate canonical authority", t => {
  ...
  db.exec(`
    ...
    CREATE TABLE labels(name TEXT PRIMARY KEY, space_id TEXT);
  ...
  db.prepare("INSERT INTO labels VALUES(?,NULL)").run("workspace");
  ...
  assert.deepEqual(
    ...
    new Set(["project", "session", "agent-profile"]),
  );
  ...
  try {
    ...
    assert.equal((adoptedDb.prepare("SELECT space_id AS spaceId FROM labels WHERE name='workspace'").get() as { spaceId: string }).spaceId, "spc_personal");

...
| Historical multi-harness matrix is a complete list of implemented harnesses | Current source contains additional provider packages, including the dedicated Command Code integration; `docs/architecture/commandcode-harness.md` records its specific invariants | Use current package registration/runtime declarations as truth; do not treat the retained table as an implementation inventory |

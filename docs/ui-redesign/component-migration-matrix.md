...
| Hand-rolled dialogs | Own `role="dialog"`/`aria-modal` in `Sidebar.tsx`, `SettingsView.tsx`, `ContextRail.tsx`, `Composer.tsx`, `CommandPalette.tsx`, `PackageTourOverlay.tsx`, `packages/task-trackers/widgets/plugin.tsx` | Migrate to `Dialog` / `Sheet` | 1b–1c |

...
origin. This design does not add remote relay, certificate pinning, VPN
discovery, or end-to-end pairing. Deployments needing internet access should
terminate TLS in front of Polyth and use the UI password.
...
- local-notification permission and delivery foundation; and
- keyboard events.
...
No production push backend is added. Existing server web push remains a web/PWA
feature; the mobile bridge can display already-derived local completion
notifications and route taps to the owning session.

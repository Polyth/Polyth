...
- If none of those proofs exists, replacement stays blocked. On first encountering a legacy uncontained ledger without proof, Polyth durably records the current host boot ID and remains blocked. Only a later boot-ID change proves that the legacy executor cannot still be running and permits a replacement generation; the observing boot itself is never treated as release proof.
...
Consumers, security and persistence impact: the authority ledger gains an optional `containment` object (`kind`, exact unit, boot ID) and an optional `legacyRecoveryBootId` migration fence. Old records remain readable. `backend-opencode` consumes the existing owned lease restart seam; session/core contracts and browser DTOs do not change. Scope names contain only a hash of authority identity and generation. System commands receive a strictly validated generated unit name, never repository or provider input.

Compatibility/migration/rollback: systems without user systemd retain receipt-only supervision and fail closed after a lost supervisor receipt during the current boot. Existing authority records have no containment; after their first blocked observation they require one real host reboot before automatic recovery. Removing the feature leaves the optional fields inert; they must not be used as proof by older code.

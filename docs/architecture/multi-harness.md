...
The composer has one model trigger. Its picker hosts the harness choices as tabs above the model catalog. Unless low-resource mode is active, the Models web package starts bounded read-only catalog discovery for every enabled harness during shell package bootstrap, without waiting for project restoration. The context-free request is server-scoped; each result is then retained in an account/Space/project page-lifetime cache under the context returned by its snapshot (the authoritative server snapshot remains cwd-scoped), and a newly restored context fills only its own missing entries. Changing a tab is therefore a browser-local next-turn choice that normally paints the already-warm model list synchronously; it never publishes a switch or submits a prompt. A catalog probe may open a temporary provider metadata connection, but it does not bind a canonical execution leg. The submitted message carries that route intent; only then does the server run the safe switch algorithm above, create or replace the execution runtime, and publish its transition and timeline evidence. Process-free roster metadata lets the picker render harness labels and logos before availability probes finish. Post-submit transition controls remain available for recovery and escalation, while the separate idle harness trigger is gone. On phones the same sheet also owns role, thinking, and profile controls.

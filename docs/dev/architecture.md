...
| `recap` | Optional idle-session recap + one grounded follow-up suggestion. Owns its server watcher/routes, quiet-time setting, timeline contribution, and package settings page; package lifecycle is the only enable/disable switch. |
...
- `packages/recap` (F9) — optional, disabled-by-default idle watcher. While the
  ...
  one grounded follow-up to the projection. Passive bookkeeping advances the raw
  freshness anchor without cancelling generation; newer conversation activity
  invalidates it. Disabling the package unsubscribes the watcher, clears timers,
  and prevents in-flight results from being committed.

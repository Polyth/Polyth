...
import { Button, Checkbox, Dialog } from "./ui/index.ts";
...
export default function ImportSessionsDialog({ projectId, onClose }: {
  ...
  return (
    <Dialog
      ...
      className="import-sessions-dialog"
      footer={(
        <>
          <span className="muted">{tr("importsessionsdialog.historyLoadsTheFirstTimeAnImported")}</span>
          <span className="header-spacer" />
          <Button size="sm" disabled={busy} onClick={onClose}>{tr("common.cancel")}</Button>
          <Button
            size="sm"
            variant="primary"
            busy={busy}
    ...
    >
      <p className="muted import-sessions-intro">
        {tr("importsessionsdialog.opencodeSessionsInThisWorkspaceThatPolyth")}
      </p>
      <div className="import-sessions-body">
        ...
        {!loading && items.length > 0 && (
          <>
            <Checkbox
              className="import-session-row import-session-all"
              checked={allOn}
              onChange={() => setSelected(allOn ? new Set() : new Set(items.map((s) => s.id)))}
              label={<><strong>{tr("importsessionsdialog.selectAll")}</strong> <small className="muted">{selected.size} {tr("importsessionsdialog.of")}{" "}{items.length}</small></>}
            />

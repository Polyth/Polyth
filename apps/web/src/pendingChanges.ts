...
/** A successful git status is authoritative, including its empty (clean) state. */
export function selectPendingChanges(
  status: GitStatus | null,
  toolPaths: readonly string[],
): PendingChangeSource {
  if (status) return { source: "git", paths: gitChangedFiles(status) };
  return { source: "tools", paths: [...new Set(toolPaths.map((path) => usablePath(path)).filter((path): path is string => path !== null))] };
}
